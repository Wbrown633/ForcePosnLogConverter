...
def parseHexString(hexString, chars):
    ...
    listofDec = []
    for ahex in listofHex:
        listofDec.append(int(ahex, 16))
    return listofDec
...
def plotData(listofForceData, listofPosnData, fileName, actuationnumber):
    ...
    t = range(0, 500, 2)
    ax1.plot(t, listofForceData, linestyle='-', marker='.', color='green')
    ax1.set_xlabel('Time (s)')

    ax2 = ax1.twinx()
    plt.plot(t, listofPosnData, linestyle='-', marker='.', color='red')

    ax1.set_ylabel('Force (N)')
    ax2.set_xlabel('Pos (mm)')
    ax1.set_title(fileName)
    plt.legend(['Position', 'Force'])
    fig.savefig(folderpath + "/" + fileName.split(".")[0] + "Actuation_" + str(actuationnumber) + ".png")
    plt.close()
...
def convertEncoderData(hexList):
    ...
    # In the future encoder rollover should be handled by the firmware and the index channel should be used to prevent
    # any guesswork.

    NEGScalingFactor = -.65
    POSITIVEScalingFactor = .56

    for index, hexvalue in enumerate(hexList):

        previoushexvalue = hexList[index - 1]
        diffbetweenpoints = (hexvalue - previoushexvalue)
        if diffbetweenpoints <= NEGScalingFactor * ENCODERCONVERSION:
            if hexvalue < previoushexvalue:
                revolutions += 1
            convertedList.append(hexvalue + revolutions * ENCODERCONVERSION)  # convert count based on revolutions
        elif diffbetweenpoints >= POSITIVEScalingFactor * ENCODERCONVERSION:
            if hexvalue > previoushexvalue:
                revolutions -= 1  # next point is less than current point
            convertedList.append(hexvalue + revolutions * ENCODERCONVERSION)  # same as above
        else:
            convertedList.append(hexvalue + revolutions * ENCODERCONVERSION)
...
if __name__ == '__main__':
    ...
    folderpath = "C:/temp/Plots/" + foldername # String must be the location of the folder to house all of our plot-by-date folders, 
    # here it is called 'Plots'
